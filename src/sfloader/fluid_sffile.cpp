...
SFData *fluid_sffile_open(const char *fname, const fluid_file_callbacks_t *fcbs)
{
    ...
    if((sf->sffd = static_cast<FILE *>(fcbs->fopen(fname))) == NULL)
    {
...
static int process_info(SFData *sf, int size)
{
    ...
    while(size > 0)
    {
        ...
        else if(chunkid(chunk.id) != UNKN_ID)
        {
            ...
            if(!((item.fcc = static_cast<uint32_t *>(FLUID_MALLOC(chunk.size + sizeof(uint32_t) + 1)))))
            {
                FLUID_LOG(FLUID_ERR, "Out of memory");
                return FALSE;
            }
...
int load_pgen(SFData *sf, int size)
{
    ...
    while(preset_list)
    {
        preset = static_cast<SFPreset *>(fluid_list_get(preset_list));
        ...
        while(zone_list)
        {
            zone = static_cast<SFZone*>(fluid_list_get(zone_list));
            level = 0;
            gen_list = zone->gen;
...
int load_igen(SFData *sf, int size)
{
    ...
    while(inst_list)
    {
        inst = static_cast<SFInst *>(fluid_list_get(inst_list));
...
{
    auto data = static_cast<sfvio_data_t *>(user_data);
