...
static int fluid_rvoice_eventhandler_push_LOCAL(fluid_rvoice_eventhandler_t *handler, const fluid_rvoice_event_t *src_event)
{
    ...
    event = static_cast<fluid_rvoice_event_t *>(fluid_ringbuffer_get_inptr(handler->queue, old_queue_stored));
...
{
    auto vptr = static_cast<fluid_rvoice_t **>(fluid_ringbuffer_get_inptr(eventhandler->finished_voices, 0));
...
fluid_rvoice_eventhandler_dispatch_all(fluid_rvoice_eventhandler_t *handler)
{
    ...
    while(NULL != (event = static_cast<fluid_rvoice_event_t *>(fluid_ringbuffer_get_outptr(handler->queue))))
    {
