...
fluid_file_read_full(fluid_file fp, size_t *length)
{
    ...
    buffer = static_cast<char *>(FLUID_MALLOC(buflen));
...
fluid_midi_file_read_event(fluid_midi_file *mf, fluid_track_t *track)
{
    ...
    if(status == MIDI_SYSEX)    /* system exclusive */
    {
        ...
        if(mf->varlen)
        {
            FLUID_LOG(FLUID_DBG, "%s: %d: alloc metadata, len = %d", __FILE__,
                      __LINE__, mf->varlen);
            metadata = static_cast<unsigned char *>(FLUID_MALLOC(mf->varlen + 1));
    ...
    else if(status == MIDI_META_EVENT)      /* meta events */
    {
        ...
        else
        {
            FLUID_LOG(FLUID_DBG, "%s: %d: alloc metadata, len = %d", __FILE__,
                      __LINE__, mf->varlen);
            dyn_buf = static_cast<unsigned char *>(FLUID_MALLOC(mf->varlen + 1));
        ...
        case MIDI_TRACK_NAME:
            metadata[mf->varlen] = 0;
            fluid_track_set_name(track, reinterpret_cast<char *>(metadata));
            break;
        ...
        case MIDI_TIME_SIGNATURE:
            ...
            denominator = pow(2.0, static_cast<double>(metadata[1]));
            clocks = metadata[2];
            notes = metadata[3];
...
fluid_track_set_name(fluid_track_t *track, char *name)
{
    ...
    track->name = static_cast<char *>(FLUID_MALLOC(len + 1));
...
{
    fluid_player_t *player = static_cast<fluid_player_t *>(data);
...
{
    auto pi = static_cast<fluid_playlist_item *>(FLUID_MALLOC(sizeof(fluid_playlist_item)));
