...
new_fluid_synth(fluid_settings_t *settings)
{
    ...
    synth->msgs_note_cut_mode = static_cast<fluid_msgs_note_cut>(i);
...
delete_fluid_synth(fluid_synth_t *synth)
{
    ...
    {
        sfont = static_cast<fluid_sfont_t *>(fluid_list_get(list));
    ...
    {
        auto timer = static_cast<fluid_timer_t *>(fluid_list_get(list));
...
fluid_synth_cc_LOCAL(fluid_synth_t *synth, int channum, int num)
{
    ...
    case DATA_ENTRY_MSB: /* not allowed to modulate (spec SF 2.01 - 8.2.1) */
    {
        ...
        if(chan->nrpn_active)   /* NRPN is active? */
        {
                    ...
                    && (fluid_channel_get_cc(chan, NRPN_LSB) < 100))
            {
                ...
                chan->nrpn_select = static_cast<fluid_gen_type>(0);  /* Reset to 0 */
            }
    ...
    case NRPN_MSB: /* not allowed to modulate (spec SF 2.01 - 8.2.1) */
        ...
        chan->nrpn_select = static_cast<fluid_gen_type>(0);
        chan->nrpn_active = 1;
        break;
    ...
    case NRPN_LSB: /* not allowed to modulate (spec SF 2.01 - 8.2.1) */
        ...
        if(fluid_channel_get_cc(chan, NRPN_MSB) == 120)
        {
            if(value == 100)
            {
                chan->nrpn_select = static_cast<fluid_gen_type>(static_cast<int>(chan->nrpn_select) + 100);
            }
            else if(value == 101)
            {
                chan->nrpn_select = static_cast<fluid_gen_type>(static_cast<int>(chan->nrpn_select) + 1000);
            }
            else if(value == 102)
            {
                chan->nrpn_select = static_cast<fluid_gen_type>(static_cast<int>(chan->nrpn_select) + 10000);
            }
            else if(value < 100)
            {
                chan->nrpn_select = static_cast<fluid_gen_type>(static_cast<int>(chan->nrpn_select) + value);
            }
        }
...
fluid_synth_update_polyphony_LOCAL(fluid_synth_t *synth, int new_polyphony)
{
    ...
    if(new_polyphony > synth->nvoice)
    {
        /* Create more voices */
        auto new_voices =
        static_cast<fluid_voice_t **>(FLUID_REALLOC(synth->voice, sizeof(fluid_voice_t *) * new_polyphony));
...
{
    int i, j, k;
    int16_t *left_out = static_cast<int16_t *>(lout);
    int16_t *right_out = static_cast<int16_t *>(rout);
    int di = *dither_index;
    fluid_profile_ref_var(prof_ref);
...
{
    auto sfont = static_cast<fluid_sfont_t *>(data);
...
fluid_synth_remove_sfont(fluid_synth_t *synth, fluid_sfont_t *sfont)
{
    ...
    for(list = synth->sfont; list; list = fluid_list_next(list))
    {
        sfont_tmp = static_cast<fluid_sfont_t *>(fluid_list_get(list));
...
fluid_synth_set_interp_method(fluid_synth_t *synth, int chan, int interp_method)
{
    ...
    for(i = 0; i < synth->midi_channels; i++)
    {
        ...
        {
            fluid_channel_set_interp_method(synth->channel[i], static_cast<fluid_interp>(interp_method));
        }
    }
...
fluid_synth_handle_midi_event(void *data, fluid_midi_event_t *event)
{
    ...
    case MIDI_SYSEX:
        return fluid_synth_sysex(synth, static_cast<const char *>(event->paramptr), event->param1, NULL, NULL, NULL, FALSE);
...
int fluid_synth_set_channel_type(fluid_synth_t *synth, int chan, int type)
{
    ...
    synth->channel[chan]->channel_type = static_cast<fluid_midi_channel_type>(type);
...
int fluid_synth_set_custom_filter(fluid_synth_t *synth, int type, int flags)
{
    ...
    synth->custom_filter_type = static_cast<fluid_iir_filter_type>(type);
    synth->custom_filter_flags = static_cast<fluid_iir_filter_flags>(flags);

    for(i = 0; i < synth->polyphony; i++)
    {
        voice = synth->voice[i];

        fluid_voice_set_custom_filter(voice, static_cast<fluid_iir_filter_type>(type), static_cast<fluid_iir_filter_flags>(flags));
    }
...
static int fluid_synth_set_important_channels(fluid_synth_t *synth, const char *channels)
{
    ...
    {
        scores->important_channels = static_cast<char *>(
        FLUID_REALLOC(scores->important_channels, sizeof(*scores->important_channels) * synth->midi_channels));
...
int fluid_synth_set_legato_mode(fluid_synth_t *synth, int chan, int legatomode)
{
    ...
    FLUID_API_ENTRY_CHAN(FLUID_FAILED);
    /**/
    synth->channel[chan]->legatomode = static_cast<fluid_channel_legato_mode>(legatomode);
    /**/
    FLUID_API_RETURN(FLUID_OK);
}
                                    ...
                                    int portamentomode)
{
    ...
    FLUID_API_ENTRY_CHAN(FLUID_FAILED);
    /**/
    synth->channel[chan]->portamentomode = static_cast<fluid_channel_portamento_mode>(portamentomode);
    /**/
    FLUID_API_RETURN(FLUID_OK);
}
