...
fluid_handle_info(void *d, int ac, char **av, fluid_ostream_t out)
{
    ...
    case FLUID_STR_TYPE:
    {
        ...
        const char *s_const = s;
        fluid_settings_getstr_default(settings, av[0], &s_const);
        fluid_ostream_printf(out, "Default value: %s\n", s);
...
fluid_cmd_handler_handle(void *data, int ac, char **av, fluid_ostream_t out)
{
    ...
    cmd = static_cast<fluid_cmd_t *>(fluid_hashtable_lookup(handler->commands, av[0]));
...
static void fluid_server_close(fluid_server_t *server)
{
    ...
    while(list)
    {
        client = static_cast<fluid_client_t *>(fluid_list_get(list));
