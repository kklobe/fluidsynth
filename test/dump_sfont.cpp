...
static void dump_preset(fluid_preset_t *preset)
{
    ...
    auto defpreset = static_cast<fluid_defpreset_t *>(fluid_preset_get_data(preset));
    if (defpreset == NULL)
    {
        return;
    }
...
static int inst_compare_func(const void *a, const void *b)
{
    const auto inst_a = static_cast<const fluid_inst_t *>(a);
    const auto inst_b = static_cast<const fluid_inst_t *>(b);
