...
static void test_default_polyphony(fluid_settings_t* settings, int with_rendering)
{
    ...
    TEST_ASSERT(synth->fonts_to_be_unloaded != NULL);
    TEST_ASSERT(fluid_timer_is_running(
    static_cast<const fluid_timer_t *>(fluid_list_get(synth->fonts_to_be_unloaded))));
    ...
    if(with_rendering)
    {
        ...
        fluid_msleep(10 * fluid_timer_get_interval(static_cast<const fluid_timer_t *>(
