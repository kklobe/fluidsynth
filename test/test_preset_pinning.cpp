...
int main(void)
{
    ...
    defsfont = static_cast<fluid_defsfont_t *>(fluid_sfont_get_data(sfont));
    defsfont->filename[0]++;
...
static int count_loaded_samples(fluid_synth_t *synth, int sfont_id)
{
    ...
    auto defsfont = static_cast<fluid_defsfont_t *>(fluid_sfont_get_data(sfont));

    for(list = defsfont->sample; list; list = fluid_list_next(list))
    {
        auto sample = static_cast<fluid_sample_t *>(fluid_list_get(list));
