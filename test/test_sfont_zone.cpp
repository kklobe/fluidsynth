...
static void good_test_1zone_2gen_1termgen(int (*load_func)(SFData *sf, int size), SFData* sf, SFZone *zone)
{
    ...
    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone->gen, 0)));
    TEST_ASSERT(gen != NULL);
    TEST_ASSERT(gen->id == GEN_KEYRANGE);
    TEST_ASSERT(gen->amount.range.lo == 60);
    TEST_ASSERT(gen->amount.range.hi == 127);

    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone->gen, 1)));
    TEST_ASSERT(gen != NULL);
...
static void bad_test_too_short_gen_buffer(int (*load_func)(SFData *sf, int size), SFData *sf, SFZone *zone)
{
    ...
    for (i = 0; i < FLUID_N_ELEMENTS(buf_with_one_gen); i++)
    {
        ...
        gen = static_cast<SFGen *>(fluid_list_get(fluid_list_nth(zone->gen, 0)));
        TEST_ASSERT(gen == NULL);
    ...
    for (i = 0; i < FLUID_N_ELEMENTS(buf_with_two_gen); i++)
    {
        ...
        FLUID_FREE(gen);
        zone->gen->data = NULL;
        gen = static_cast<SFGen *>(fluid_list_get(fluid_list_nth(zone->gen, 1)));
        TEST_ASSERT(gen == NULL);
...
static void bad_test_gen_wrong_order(int (*load_func)(SFData *sf, int size), SFData *sf, SFZone *zone)
{
    ...
    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone->gen, 2)));
    TEST_ASSERT(gen == NULL);
...
static void bad_test_issue_808(int (*load_func)(SFData *sf, int size), SFData *sf, SFZone *zone1)
{
    ...
    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 0)));
    TEST_ASSERT(gen != NULL);
    TEST_ASSERT(gen->id == GEN_REVERBSEND);
    TEST_ASSERT(gen->amount.uword == 50);

    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 1)));
    TEST_ASSERT(gen != NULL);
    TEST_ASSERT(gen->id == GEN_VOLENVRELEASE);
    TEST_ASSERT(gen->amount.uword == 0xF9CE);

    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 2)));
    TEST_ASSERT(gen == NULL);
...
static void default_key_range_test_issue_1250(int (*load_func)(SFData *sf, int size), SFData *sf, SFZone *zone1, SFZone *zone2)
{
    ...
    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone2->gen, 0)));
    TEST_ASSERT(gen != NULL);
    TEST_ASSERT(gen->id == GEN_PAN);
    TEST_ASSERT(gen->amount.uword == 50);

    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone2->gen, 1)));
    TEST_ASSERT(gen != NULL);
    TEST_ASSERT(gen->id == GEN_SAMPLEMODE);
    TEST_ASSERT(gen->amount.uword == 1);

    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone2->gen, 2)));
    TEST_ASSERT(gen != NULL);
    TEST_ASSERT(gen->id == GEN_SAMPLEID);
    TEST_ASSERT(gen->amount.uword == 0);

    gen = static_cast<const SFGen *>(fluid_list_get(fluid_list_nth(zone2->gen, 3)));
    TEST_ASSERT(gen == NULL);
...
static void bad_test_additional_gens_after_final_gen(int (*load_func)(SFData *sf, int size), SFData *sf, SFZone *zone1)
{
    ...
    for (i = 0; i < FLUID_N_ELEMENTS(buf); i++)
    {
        ...
        gen = static_cast<SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 0)));
        TEST_ASSERT(gen != NULL);
        TEST_ASSERT(gen->id == GEN_KEYRANGE);
        TEST_ASSERT(gen->amount.range.lo == 60);
        TEST_ASSERT(gen->amount.range.hi == 127);
        ...
        gen = static_cast<SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 1)));
        TEST_ASSERT(gen != NULL);
        if (load_func == &load_igen)
        {
            TEST_ASSERT(gen->id == GEN_SAMPLEID);
        ...
        gen = static_cast<SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 2)));
        TEST_ASSERT(gen == NULL);

        gen = static_cast<SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 3)));
        TEST_ASSERT(gen == NULL);

        gen = static_cast<SFGen *>(fluid_list_get(fluid_list_nth(zone1->gen, 4)));
        TEST_ASSERT(gen == NULL);
